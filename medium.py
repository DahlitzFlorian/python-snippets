...
def get_list_of_followings(user_id):
    ...
    while True:
        ...
        if next_id:
            ...
            url = (
                MEDIUM + "/_/api/users/" + user_id + "/following?limit=8&to=" + next_id
            )
        else:
...
def get_post_responses(posts):
    ...
    for post in posts:
        ...
        sleep(
            0.5
        )  # This is the most intensive operation for the Medium servers, we'll help them out
...
@click.command()
@click.option("-n", "--name", default="DahlitzF", help="Medium username")
@click.option(
    "-r",
    "--min-recommendations",
    default=10,
    help="Minimum number of recommendations per response",
)
