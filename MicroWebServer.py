...
class WebServer(object):
    def __init__(self, hostname='', port=80, document_root='./'):
        self._connectionHandler = WebServer.ConnectionHandler(hostname, port, document_root)
    ...
    class ConnectionHandler(threading.Thread):
        ...
        def run(self):
            ...
            while self.running:
                ...
                print('%s [%s] requested %s%s' % (address[0], http_request.header['User-Agent'], http_request.host,
                                                  http_request.uri))
                ...
                if os.path.isfile(self._document_root + file):
                    with open(self._document_root + file, 'rb') as f:
                        http_response = WebServer.HTTPResponse(
                            f.read(),
                            content_type=WebServer.HTTPResponse.file_extension_to_content_type(file.split('.')[-1])
                        )
                        f.close()

                elif os.path.isfile(self._document_root + 'error404.html'):
                    with open(self._document_root + 'error404.html', 'rb') as f:
                        http_response = WebServer.HTTPResponse(f.read(), code='404 NOT FOUND')
                        f.close()
